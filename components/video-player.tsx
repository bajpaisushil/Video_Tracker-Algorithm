...
export default function VideoPlayer({ videoSrc, videoTitle }: VideoPlayerProps) {
  ...
  useEffect(() => {
    if (progress > 0 || watchedIntervals.length > 0) {
      const session: WatchSession = {
        currentTime,
        watchedIntervals,
        progress,
      }
      localStorage.setItem(`${userId}-${videoId.current}`, JSON.stringify(session))
    }
  ...
  }, [isPlaying, currentInterval, watchedIntervals])
  ...
  return (
    <div className="w-full bg-white rounded-lg shadow-lg overflow-hidden">
      ...
      <div className="relative">
        <video ref={videoRef} className="w-full aspect-video bg-black" src={videoSrc} playsInline />
        ...
        <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/80 to-transparent p-4">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="text-white text-sm font-medium">{Math.round(progress)}% completed</span>
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-sm text-gray-500">{Math.round(progress)}% unique content watched</span>
        </div>
        ...
        <p className="mt-4 text-sm text-gray-600">
          This progress tracker only counts unique segments of the video you've watched. Rewatching the same parts won't
